...
def pCc_conditional(c_range, b, s, t, tree):
    ...
    :param b: branch id of the recombination event
...
def pCc(c_range, t, tree, verbose=False):
    ...
    T2 = tree.tim_lineages[0] / N
    indb = 0
    with tqdm(total=(2 * t.num_samples() - 1)) as pbar:
        for b in t.nodes():
            if b != t.root:
                if verbose:
                    print("-" * 15)
                ...
                for indc, c in enumerate(c_range):
                    if c == 0:
                        ...
                        if c < 0:
                            ...
                            P = P - (np.exp((ll - 1) * (Tl - tup - c)) - 1) / (ll - 1)
    ...
    for b in t.nodes():
        if b != t.root:
            ...
            if t.parent(b) == t.root:
                for k in range(nup + 1, ndown + 1):
                    P += branchdurations.Q1(k, tree) + branchdurations.Q2(
                        k, k, b, t, tree
                    )
            else:
                P += tup - tdown
                for k in range(nup + 1, ndown + 1):
                    P -= branchdurations.Q3(k, tree)
    return P / tree.tbl
...
def pDnegative(t, tree):
    ...
    for b in t.children(t.root):
        ...
        for k in range(2, ndown + 1):
            P += (
                (k - 1) * branchdurations.Q1(k, tree)
                - branchdurations.Q2(k, k, b, t, tree)
                - branchdurations.Q3(k, tree)
            )
    return P / tree.tbl
...
def pCnegative(t, tree):
    """
    Probability that the total branch length decreases
    :param t: tree in tskit format
    :param tree: tree object
    :return:
    """
    ...
    for b in t.nodes():
        if b != t.root:
            ...
            ndown = tree.lookup_ns(tdown)
            for k in range(nup + 1, ndown + 1):
                P += (k - 1) * branchdurations.Q1(k, tree) + branchdurations.Q2kxyAB(
                    k, k, nup + 1, 1, -1, tree
                )
    return P / tree.tbl
...
def pCzero(t, tree):
    """
    Probability that the total branch length does not change
    :param t: tree in tskit format
    :param tree: tree object
    :return:
    """
    ...
    for b in t.nodes():
        if b != t.root:
            ...
            ndown = tree.lookup_ns(tdown)
            for k in range(nup + 1, ndown + 1):
                P += branchdurations.Q1(k, tree) + branchdurations.Q2kxyAB(
                    k, k, nup + 1, 0, 1, tree
                )
    return P / tree.tbl
...
def pCpositive(t, tree):
    """
    Probability that the total branch length increases
    :param t: tree in tskit format
    :param tree: tree object
    :return:
    """
    ...
    for b in t.nodes():
        if b != t.root:
            ...
            ndown = tree.lookup_ns(tdown)
            for k in range(nup + 1, ndown + 1):
                P += branchdurations.Q2kxyAB(
                    k, nup, 2, 1, 0, tree
                ) + branchdurations.Q3(k, tree)
    return P / tree.tbl


def mean_change(ts, Ne, sim_type="height", save_to_file=None):
    """
    Calculate the mean change in the statistic of interest (height or total branch length)
    :param ts: tree sequence in tskit format
    :param Ne: effective population size (diploid)
    :param sim_type: "height" or "tbl"
    :param save_to_file: path to save plot of results
    :return:
    """
    ...
    if save_to_file is not None:
        viz.mean_change_plot(results_ranked, sim_type, save_to_file=save_to_file)
...
def mean_change_sim(
    ...
    """
    Simulate trees and calculate the mean change in the statistic of interest (height or total branch length)
    :param sim_type: "height" or "tbl"
    ...
    :param save_to_file: path to save results plot
    :return:
    """
    ...
    while np.sum(results_ranked[0]) != n_per_bin * (bins_end - bins_start + 1):
        ...
        trees = branchdurations.compute_trees(ts, 1.0, quiet=True)
        t = ts.first()
        tree = trees.trees[0]
        if sim_type == "height":
    ...
    if save_to_file is not None:
        viz.mean_change_plot(results_ranked, sim_type, save_to_file, size=size)
