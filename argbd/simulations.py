...
def simulate_data(contig, model, samples, sim_model="hudson", rs=-1):
    if rs == -1:
    ...
    ts = engine.simulate(
        model, contig, samples, msprime_model=sim_model, random_seed=rs
    )
...
def run_relate(
    ...
    if not quiet:
        ...
        print("Converting files for Relate...", end="")
        os.system(
            relate_loc
            + "/bin/RelateFileFormats --mode ConvertFromVcf --haps "
            + S
        ...
        if consistency:
            ...
            So = S
        print(
            "Relate --mode All --sample_ages sample_ages.txt "
            + cons
            + "-- memory "
            + str(memory)
            + " -m "
...
def read_mut_file(filename, sequence_length):
    ...
    with open(filename + ".mut", "r") as file:
        ...
        for i, line in enumerate(file.readlines()):
            ...
            ch5 = ch[5].split(" ")
            if not ch5[0]:
                # Mutation is not mapped to any branch, skip
                continue
            else:
                branch_indices = [int(b) for b in ch5]
                if len(branch_indices) == 1:
                    edges[int(ch[1])] = (int(ch[4]), branch_indices[0])
...
def read_anc_file(
    ...
    :param filename: filehandle, no extension
    :param sequence_length: sequence length
    :return: edges_relate is a dict of edge IDs {unique branch : id in tskit tree}
    ...
    print("Unique branches:", kk, "of which", k, "have at least one mutation")
...
def run_tsinfer(ts, Ne, contig):
    ...
    for s in ts_tsinfer.sites():
        if len(s.mutations) == 1:
            num_muts += 1
    print("Mutations mapping uniquely to a branch:", num_muts)
    num_nodes = 0
    num_unary = 0
    num_polytomies = 0
...
def flat_recombination_map(recombination_rate, sequence_length):
    with open("dummy_map.txt", "w") as file:
        ...
        file.write(str(int(sequence_length)) + " 0.0 " + str(recombination_rate * 1e2 * sequence_length) + "\n")
